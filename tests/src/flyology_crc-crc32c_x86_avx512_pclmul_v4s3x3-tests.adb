...
package body Flyology_CRC.CRC32C_X86_AVX512_PCLMUL_V4S3X3.Tests is
   ...
   --  Test-only authority for instruction emulators whose CPUID/XCR0 model
   --  does not satisfy the production gate.  Never used by library dispatch.
   Force_Direct : constant Boolean :=
     Ada.Environment_Variables.Exists ("FLYOLOGY_CRC_FORCE_AVX512_TEST")
     and then
       Ada.Environment_Variables.Value ("FLYOLOGY_CRC_FORCE_AVX512_TEST") = "1";
   ...
   procedure Check_Case (Length, Alignment : Natural) is
      ...
      --  spans all sixteen residues required by the AVX-512 path.
   ...
   begin
      for State of States loop
         Check
           (Update (State, Data) =
              CRC32C_X86_SSE_V4S3X3.Update (State, Data),
   ...
   procedure Check_Gated_Fallback is
      type Length_List is array (Positive range <>) of Natural;
      Lengths : constant Length_List := (257, 280, 416, 1_024, 4_096);
   begin
      for Length of Lengths loop
         declare
            First : constant Offset := 19;
            Last  : constant Offset := First + Offset (Length) - 1;
            Data  : Byte_Array renames Test_Data (First .. Last);
         begin
            Check
              (Width_32.Compute_ISCSI (Data) =
                 (Table_Update (16#FFFF_FFFF#, Data) xor 16#FFFF_FFFF#),
               "feature-gated public fallback length" & Length'Image);
         end;
      end loop;
   end Check_Gated_Fallback;
   ...
   begin
      ...
      if not Compiled then
         Check (not X86_Features.AVX512_Ternary_Available,
                "non-x86 AVX-512 ternary feature is false");
         Check_Stub;
      elsif
        not (X86_Features.CRC32C_SSE_Available
             and then X86_Features.AVX512_Ternary_Available)
        and then not Force_Direct
      then
         declare
            Empty : Byte_Array renames Test_Data (17 .. 16);
         begin
            Check
              (Update (16#1357_9BDF#, Empty) = 16#1357_9BDF#,
               "x86 empty-input identity without AVX-512 ternary tier");
         end;
         Check_Gated_Fallback;
         Ada.Text_IO.Put_Line
           ("x86 CRC32C AVX-512 fusion direct tests skipped: tier unavailable");
      else
         --  Direct Update calls force this leaf rather than exercising only
         --  the public dispatch.  The band covers the 256-byte dispatch and
      ...
      if Failures /= 0 then
         raise Program_Error with Failures'Image & " x86 AVX-512 fusion tests failed";
      end if;
      ...
      if Compiled
        and then
          ((X86_Features.CRC32C_SSE_Available
            and then X86_Features.AVX512_Ternary_Available)
           or else Force_Direct)
      then
         Ada.Text_IO.Put_Line
           ("x86 CRC32C AVX-512 fusion matches table, public, alignment, and chaining cases");
      elsif not Compiled then
         Ada.Text_IO.Put_Line
           ("x86 CRC32C AVX-512 fusion non-x86 stubs fail closed as required");
      end if;
   end Run;
end Flyology_CRC.CRC32C_X86_AVX512_PCLMUL_V4S3X3.Tests;
