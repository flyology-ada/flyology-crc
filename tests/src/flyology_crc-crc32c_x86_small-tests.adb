...
package body Flyology_CRC.CRC32C_X86_Small.Tests is
   ...
   procedure Check_Case (Length, Alignment : Natural) is
      --  The nonzero lower bound is intentional: it checks that Update uses
      --  Data'Address/Data'Length rather than assuming an index origin.
   ...
   procedure Run is
      ...
      Chained_Lengths : constant Length_List :=
        (0, 1, 7, 8, 63, 64, 65, 127, 128, 255, 256, 257);
   begin
      for Index in Test_Data'Range loop
         Test_Data (Index) :=
           Byte
             ((Natural (Index) * 149 + Natural (Index) / 5 + 16#63#) mod 256);
      end loop;
      ...
      if not Compiled then
         ...
         begin
            Check
              (Update (16#1357_9BDF#, Empty) = 16#1357_9BDF#,
               "x86 empty-input identity without SSE4.2");
         end;
         Ada.Text_IO.Put_Line ("x86 CRC32C small tests skipped: SSE tier unavailable");
      else
         for Length in 0 .. 257 loop
      ...
      if Failures /= 0 then
         raise Program_Error with Failures'Image & " x86 CRC32C small tests failed";
      end if;

      if Compiled and then X86_Features.CRC32C_SSE_Available then
         Ada.Text_IO.Put_Line
           ("x86 CRC32C small kernel matches table, public, and chaining results");
      elsif not Compiled then
         Ada.Text_IO.Put_Line ("x86 CRC32C small non-x86 stubs fail closed as required");
      end if;
   end Run;
end Flyology_CRC.CRC32C_X86_Small.Tests;
